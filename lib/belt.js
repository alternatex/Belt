...
(function(){	
	...
	// action-hooks helper
	var Action = {
		hook: function(advice, options, callback){
			// validate 
			if(((type = typeof options.target) &&  typeof ({'function': true, 'object': true})[type]!='undefined' ? type : false)!==false) {
				// extract class | instance 
				var reference = (type=='function') ? options.target.prototype : options.target;
				// wrap
				(function(target, method, reference, delegate){															
					reference[method] = function(){
						// pre-process
						if(advice=='before') callback.apply(target, arguments);						
						// process
						var returnValue = delegate.apply(target, arguments);																	
						// post-process - TODO: retval mod
						if(advice=='after') callback.apply(target, arguments); 							
						// ...
						return returnValue;
					};
				})(options.target, options.method, reference, reference[options.method]);
			}								
		},
